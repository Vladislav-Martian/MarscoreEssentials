...
namespace Marscore.Essentials.Utils
{
    ...
    public class FileModel
    {
        ...
        {
            XORProcessor.ProcessInPlace(Content, key);
            Name = Encoding.UTF8.GetString(XORProcessor.ProcessInPlace(Encoding.UTF8.GetBytes(Name), key));
            Extension = Encoding.UTF8.GetString(XORProcessor.ProcessInPlace(Encoding.UTF8.GetBytes(Extension), key));
        }
        ...
        public FileModel DecryptAndUnpack(in byte[] key)
        {
            ...
            ms.Position = 0;
            var res = (FileModel)_formatter.Deserialize(ms);
            res.Recrypt(key);
            return res;
        }
    }
